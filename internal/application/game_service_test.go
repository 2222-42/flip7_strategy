...
func TestRoundRobinDealerRotation(t *testing.T) {
	...
	p1 := domain.NewPlayer("P1", &MockStrategy{DecideResult: domain.TurnChoiceStay})
	p2 := domain.NewPlayer("P2", &MockStrategy{DecideResult: domain.TurnChoiceStay})
	p3 := domain.NewPlayer("P3", &MockStrategy{DecideResult: domain.TurnChoiceStay})
	players := []*domain.Player{p1, p2, p3}
	game := domain.NewGame(players)
	svc := application.NewGameService(game)
	svc.Silent = true

	// We want to verify that DealerIndex increments correctly across rounds.
	// We can't easily run full rounds without a rigged deck or strategy that ensures quick rounds.
	// However, we can inspect the Game state after manually invoking logic or by mocking PlayRound behavior if we could.
	// Since we can't mock PlayRound (it's a method on the struct we are testing), we have to rely on the state changes.

	// Let's manually trigger the rotation logic that is inside RunGame loop.
	// But RunGame is a loop.
	// We can simulate the loop body steps.
	...
	// Verify ActivePlayers order: P1, P2, P3
	...
	// 4. Rotate Dealer (Should wrap to P1)
	game.DealerIndex = (game.DealerIndex + 1) % len(game.Players)
	if game.DealerIndex != 0 {
